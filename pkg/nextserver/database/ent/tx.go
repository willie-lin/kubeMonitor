...
func (tx *Tx) init() {
	...
	tx.IncidentBasicRule = NewIncidentBasicRuleClient(tx.config)
	tx.K8sCluster = NewK8sClusterClient(tx.config)
	tx.K8sConnector = NewK8sConnectorClient(tx.config)
	tx.K8sContainer = NewK8sContainerClient(tx.config)
	tx.K8sDaemonSet = NewK8sDaemonSetClient(tx.config)
	tx.K8sDeployment = NewK8sDeploymentClient(tx.config)
	tx.K8sEvent = NewK8sEventClient(tx.config)
	tx.K8sLabel = NewK8sLabelClient(tx.config)
	tx.K8sMetric = NewK8sMetricClient(tx.config)
	tx.K8sNamespace = NewK8sNamespaceClient(tx.config)
	tx.K8sNode = NewK8sNodeClient(tx.config)
	tx.K8sObject = NewK8sObjectClient(tx.config)
	tx.K8sObjectTag = NewK8sObjectTagClient(tx.config)
	tx.K8sPod = NewK8sPodClient(tx.config)
	tx.K8sReplicaSet = NewK8sReplicaSetClient(tx.config)
	tx.K8sStatefulSet = NewK8sStatefulSetClient(tx.config)
	tx.Metric = NewMetricClient(tx.config)
	tx.MetricEndpoint = NewMetricEndpointClient(tx.config)
	tx.MetricLabel = NewMetricLabelClient(tx.config)
	tx.MetricName = NewMetricNameClient(tx.config)
