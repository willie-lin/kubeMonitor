...
// Where adds a new predicate for the K8sClusterQuery builder.
...
func (kcq *K8sClusterQuery) Unique(unique bool) *K8sClusterQuery {
	kcq.unique = &unique
	return kcq
}
...
// First returns the first K8sCluster entity from the query.
// Returns a *NotFoundError when no K8sCluster was found.
...
// FirstID returns the first K8sCluster ID from the query.
// Returns a *NotFoundError when no K8sCluster ID was found.
func (kcq *K8sClusterQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (kcq *K8sClusterQuery) FirstIDX(ctx context.Context) uint {
...
// Only returns a single K8sCluster entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when exactly one K8sCluster entity is not found.
// Returns a *NotFoundError when no K8sCluster entities are found.
...
// OnlyID is like Only, but returns the only K8sCluster ID in the query.
// Returns a *NotSingularError when exactly one K8sCluster ID is not found.
// Returns a *NotFoundError when no entities are found.
func (kcq *K8sClusterQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (kcq *K8sClusterQuery) OnlyIDX(ctx context.Context) uint {
...
func (kcq *K8sClusterQuery) IDs(ctx context.Context) ([]uint, error) {
	var ids []uint
...
func (kcq *K8sClusterQuery) IDsX(ctx context.Context) []uint {
	ids, err := kcq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// Clone returns a duplicate of the K8sClusterQuery builder, including all associated steps. It can be
...
//		GroupBy(k8scluster.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.K8sCluster.Query().
//		Select(k8scluster.FieldCreatedAt).
//		Scan(ctx, &v)
//
func (kcq *K8sClusterQuery) Select(fields ...string) *K8sClusterSelect {
	kcq.fields = append(kcq.fields, fields...)
	return &K8sClusterSelect{K8sClusterQuery: kcq}
}
...
func (kcq *K8sClusterQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := kcq.fields; len(fields) > 0 {
		...
		for i := range fields {
			if fields[i] != k8scluster.FieldID {
...
func (kcq *K8sClusterQuery) sqlQuery(ctx context.Context) *sql.Selector {
...
// K8sClusterGroupBy is the group-by builder for K8sCluster entities.
...
// K8sClusterSelect is the builder for selecting fields of K8sCluster entities.
...
func (kcs *K8sClusterSelect) Scan(ctx context.Context, v interface{}) error {
	...
	kcs.sql = kcs.K8sClusterQuery.sqlQuery(ctx)
