...
func (cq *ClusterQuery) QueryAgents() *AgentQuery {
	query := &AgentQuery{config: cq.config}
...
func (cq *ClusterQuery) QueryNodes() *NodeQuery {
...
func (cq *ClusterQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (cq *ClusterQuery) FirstIDX(ctx context.Context) uint {
...
func (cq *ClusterQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (cq *ClusterQuery) OnlyIDX(ctx context.Context) uint {
...
func (cq *ClusterQuery) IDs(ctx context.Context) ([]uint, error) {
	var ids []uint
...
func (cq *ClusterQuery) IDsX(ctx context.Context) []uint {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *ClusterQuery) Clone() *ClusterQuery {
	...
	return &ClusterQuery{
		...
		withAgents: cq.withAgents.Clone(),
...
func (cq *ClusterQuery) WithAgents(opts ...func(*AgentQuery)) *ClusterQuery {
	query := &AgentQuery{config: cq.config}
...
func (cq *ClusterQuery) WithNodes(opts ...func(*NodeQuery)) *ClusterQuery {
	query := &NodeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ClusterQuery) sqlAll(ctx context.Context) ([]*Cluster, error) {
	var (
		nodes       = []*Cluster{}
		_spec       = cq.querySpec()
		loadedTypes = [2]bool{
	...
	if query := cq.withAgents; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint]*Cluster)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Agents = []*Agent{}
		}
		query.withFKs = true
		query.Where(predicate.Agent(func(s *sql.Selector) {
			s.Where(sql.InValues(cluster.AgentsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.cluster_agents
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "cluster_agents" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "cluster_agents" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Agents = append(node.Edges.Agents, n)
		}
	}
	...
	if query := cq.withNodes; query != nil {
		...
		query.Where(predicate.Node(func(s *sql.Selector) {
			s.Where(sql.InValues(cluster.NodesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.cluster_nodes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "cluster_nodes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "cluster_nodes" returned %v for node %v`, *fk, n.ID)
			}
...
func (cq *ClusterQuery) sqlExist(ctx context.Context) (bool, error) {
...
func (cgb *ClusterGroupBy) sqlScan(ctx context.Context, v interface{}) error {
...
func (cgb *ClusterGroupBy) sqlQuery() *sql.Selector {
...
func (cs *ClusterSelect) Scan(ctx context.Context, v interface{}) error {
	...
	cs.sql = cs.ClusterQuery.sqlQuery(ctx)
...
func (cs *ClusterSelect) sqlScan(ctx context.Context, v interface{}) error {
	rows := &sql.Rows{}
